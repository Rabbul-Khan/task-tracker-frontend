...
<template>
  <div class="flex bg-white max-w-[500px] w-full rounded-2xl shadow-sm flex-col justify-center p-8 md:p-16 lg:p-20">
    ...
    <UForm :schema="schema" :state="state" class="space-y-6 flex flex-col" :validate-on="['change', 'input']" @submit="handleLogin">
      ...
      <NuxtLink to="#" class="text-primary   hover:underline hover:text-primary text-right text-sm">
        Forgot Password?
      </NuxtLink>
      ...
      <p class="text-center text-sm text-text-secondary">
        ...
        <NuxtLink to="/register" class="text-primary font-medium  hover:underline hover:text-primary">
          Sign Up
        </NuxtLink>
      </p>
    </UForm>
